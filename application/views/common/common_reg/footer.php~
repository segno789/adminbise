...
<script type="text/javascript" src="<?php echo base_url(); ?>assets/js/jquery.pwstrength.min.js"></script>
<?php 
//DebugBreak();
if(isset($files)){
...
function validpwd()
    {
      ...
      var Pwd = $('#Pwd').val();
      var Pwd1 = $('#Pwd1').val();
           //alertify.success("oldPwd.length = "+oldPwd.length);
      if(oldPwd == "")
      {
        alertify.error("Please write your old Password");
        $('#oldPwd').focus();
        return false;
      }
      else if(Pwd == "")
      {
       alertify.error("Please write your New Password");
        $('#Pwd').focus();
        return false;
      }
      else if(Pwd1 == "")
      ...
      else if(oldPwd.length < 3)
      {
        alertify.error("Please write your old correct Password");
        $('#oldPwd').focus();
        return false;
      }
      else if(Pwd.length < 7)
      {
        alertify.error("Please write your New Password with Minimum 8 length");
        $('#Pwd').focus();
        return false;
      }
      else if(Pwd != Pwd1)
      {
        alertify.error("Your New Password and Confirm Password not match!");
        $('#Pwd').focus();
        return false;
      }
      alertify.success("You can proceed further");
       return true;
...
if(isReadm == 0)
{
    $( "#dob" ).datepicker({ dateFormat: 'dd-mm-yy',changeMonth: true, changeYear: true,maxDate: '0', minDate:new Date(1983, 0, 1)}).val();   //maxDate:new Date(2005, 7, 1),
}
    ...
    $( "#corr_dob" ).datepicker({ dateFormat: 'dd-mm-yy',changeMonth: true, changeYear: true,maxDate: '0', minDate:new Date(1983, 0, 1)}).val(); //maxDate:new Date(2005, 7, 1),
