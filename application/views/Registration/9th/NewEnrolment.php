            ...
            <div class="span12">
                <div class="widget">
                    ...
                    <div class="widget-body">
                        ...
                        <form class="form-horizontal no-margin" action="<?php echo base_url(); ?>Registration/NewEnrolment_insert" method="post" enctype="multipart/form-data" id="myform">
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls controls-row">
                                    ...
                                    <select id="sub8"  name="sub8" class="span3 dropdown">
                                        <option value="<?php  if (($excep != "") && ($excep['excep'] != "success")){ echo $excep['sub8'];} else {echo "4";} ?>"><?php if (($excep != "") && ($excep['excep'] != "success")){ echo array_search($excep['sub8'],$subarray);}else{echo "Pakistan Studies";} ?></option>
                                    </select>
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls controls-row">
                                     <select id="sub4"  name="sub4" class="span3 dropdown"><option value="<?php if (($excep != "") && ($excep['excep'] != "success")){ echo $excep['sub5'];}  ?>"><?php if (($excep != "") && ($excep['excep'] != "success")){ echo array_search($excep['sub5'],$subarray);}  ?></option></select>
                              ...
                              <div class="control-group">
                                ...
                                <div class="controls controls-row">
                                    <select id="sub6"  name="sub6" class="span3 dropdown">
                                        <option value="<?php if (($excep != "") && ($excep['excep'] != "success")){ echo $excep['sub7'];} ?>"><?php if (($excep != "") && ($excep['excep'] != "success")){ echo array_search($excep['sub7'],$subarray);} ?></option>
                                    </select>
                                    <select id="sub7" class="span3 dropdown" name="sub7">
                                        <option value="<?php if (($excep != "") && ($excep['excep'] != "success")){ echo $excep['sub8'];} ?>"><?php if (($excep != "") && ($excep['excep'] != "success")){ echo array_search($excep['sub8'],$subarray);} ?></option></select> 
                            ...
                            <div class="form-actions no-margin">
                                ...
                                <input type="hidden" name="oldrno_reg" id="oldrno_reg" value="0"> 
                                <input type="hidden" name="oldyear_reg" id="oldyear_reg" value="0"> 
                                <input type="hidden" name="oldsess_reg" id="oldsess_reg" value="0"> 
                            </div>
...
var status = 0;
    function checks(){
        status  =  check_NewEnrol_validation();
        if(status == 0)
        {
            return false;    
        }
    ...
    function checkrno(){
        $.ajax({
            type: "POST",
            url: "<?php  echo site_url('Registration/isExist'); ?>",
            ...
            success: function(data)
            {
                ...
                {
                    //  console.log("data find Status = "+localstatus);
                    var array = obj.split(",");
                    var rno = array[0];
                    var sess = array[1];
                    var year = array[2] ;
                    var result = array[4];
                    //  console.log("rno = "+rno +"  Session = "+sess+" Year = "+year+" status = "+localstatus);
                    if(year == "<?php echo regyear; ?>")
                    {
                        alertify.error("You can only appear in Re-Admission schedule due to already registered in "+year);
                        //console.log("You can only appear in Re-Admission schedule due to already registered in "+year);
                    }
                    else if(year < "<?php echo regyear ?>" && result == 1)
                    {
                        alertify.error("You are already appeared in "+array[1]+" " +array[2] +" against Roll No. "+rno+" and Passed. You can not Registered again.");
                        //  console.log("You are already appeared in "+array[1]+" " +array[2] +" against Roll No. "+rno+" and Passed. You can not Registered again.");
                    }
                    else if(year < "<?php echo regyear ?>" && result != 1)
                    {
                        $("#oldrno_reg").val(rno);
                        $("#oldyear_reg").val(year);
                        if(sess == "Annual")
                        {
                            $("#oldsess_reg").val(1);
                        }
