...
class Admission_matric extends CI_Controller {
    ...
    function updatezone()
    {
        ...
        $this->load->model('Admission_matric_model');
        $Inst_Id = $userinfo['Inst_Id'];
        $isres = $this->Admission_matric_model->Incomplete_inst($_POST,$Inst_Id);
    ...
    public function ChallanForm_Adm10th_Regular()
    {
        ...
        $pdf->AliasNbPages();
        $pdf->SetTitle("Challan Form | Admission Matric ".Year."  ".$mysess." Batch Form Fee");
    ...
    public function NewEnrolment_update_matric()
    {
        ...
        $data = array(
            ...
            'SchGrade'=>$this->input->post('SchGrade'),
    ...
    public function NewEnrolment_INSERT_matric()
    {
        ...
        $allinputdata = array('cand_name'=>@$_POST['cand_name'],'father_name'=>@$_POST['father_name'],
            ...
            'medium'=>@$_POST['medium'],'nationality'=>@$_POST['nationality'],
            'gender'=>@$_POST['gend'],'hafiz'=>@$_POST['hafiz'],
            'religion'=>@$_POST['religion'],'std_group'=>@$_POST['std_group_hidden'],
            'address'=>@$_POST['address'],
            'UrbanRural'=>@$_POST['UrbanRural'],
            'SchGrade'=>@$_POST['SchGrade'],
            'dist'=>@$_POST['pvtinfo_dist'],
    ...
    public function NewEnrolment_update()
    {
        ...
        if(@$_POST['IsReAdm'] == '1')
        {
            ...
            $allinputdata = array('name'=>@$_POST['cand_name'],'Fname'=>@$_POST['father_name'],
                ...
                'sub4'=>@$_POST['sub4'],'sub5'=>@$_POST['sub5'],
                'sub6'=>@$_POST['sub6'],'sub7'=>@$_POST['sub7'],
                'sub8'=>@$_POST['sub8'],'PicPath'=>@$_POST['pic'],'formNo'=>@$_POST['formNo'],
                'sub1ap1' => ($sub1ap1),
                'sub2ap1' => ($sub2ap1),
                'sub3ap1' => ($sub3ap1),
                'sub4ap1' => ($sub4ap1),
        ...
        foreach($q1 as $k=>$v) 
        {
            ...
            {      
                ...
                if($v['cat09'] == 2)
                {
                    ...
                    if(($v['grp_cd']==1 ||$v['grp_cd']==7 || $v['grp_cd']==8) || (array_search($v['sub5'],$practical_Sub) || array_search($v['sub6'],$practical_Sub) || array_search($v['sub7'],$practical_Sub)))
                    {
...
*/
    ...
    public function forwarding_pdf()
    {
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(0.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, "Name of Institution with Address:", 0.25, "C");


        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(2.45+$x,1.+$y);
        $pdf->MultiCell(0, 0.15, $result['name']);

        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(6.2+$x,0.8+$y);
        $pdf->Cell(0, 0.25, "School Code: _____________", 0.25, "C");
        $pdf->SetXY(7.1+$x,0.8+$y);
        $pdf->Cell(0, 0.25, $user['Inst_Id'], 0.25, "C");
        ...
        $pdf->SetXY(0.9+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['teh_name'], 0.25, "C");
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(3.2+$x,1.+$y);
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(3.8+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['dist_name'], 0.25, "C");
        ...
        $total_font = 12;
        $pdf->SetFont('Arial','B',$total_font);
        $pdf->SetXY(7.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['Sci_total'], 0.25, "C");
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(1.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['emis_code'].'123456', 0.25, "C");
        ...
        $pdf->SetFont('Arial','B',$total_font);
        $pdf->SetXY(7.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['Arts_total'], 0.25, "C");
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(1.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['LandLine'], 0.25, "C");
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(4.3+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['MobNo'], 0.25, "C");
        ...
        $pdf->SetFont('Arial','B',$total_font);
        $pdf->SetXY(7.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['Grand_total'], 0.25, "C");
        ...
        $pdf->SetFont('Arial','B',9);
        $pdf->SetXY(7.4+$x,1.+$y);
        $pdf->Cell(0, 0.25, $result['zone_cd'], 0.25, "C");
    ...
    public function return_pdf()
    {
        ...
        foreach ($result as $key=>$data) 
        {
            ...
            if($countofrecords==15) {
                ...
                $pdf->SetFont('Arial','',10);
                $pdf->SetXY(1.7,0.4);
                $pdf->Cell(0, 0.25, "MATRIC PART-II ADMISSION RETURN SESSION (".CURRENT_SESS.")", 0.25, "C");
    ...
    public function Print_Admission_matric_Form_Proofreading_Groupwise()
    {
        ...
        foreach ($result as $key=>$data) 
        {
            ...
            $pdf->Cell(0.5,0.5,  $data['sub8ap1'] != 1 ? '':   '    '. $data['sub8_NAME'],0,'L');
    ...
    public function financeReoprt()
    {
        ...
        $pdf->SetXY($x-80, $Y+53);
        $pdf->Cell(0,0,$specFee1,0,0,'L',0);
        $pdf->SetXY($x-80, $Y+55.5);
        $pdf->Cell(0,0,$specFee2,0,0,'L',0);

        $pdf->SetFont('Arial','B',$font);
        $pdf->SetXY($x-30, $Y+180);
        $pdf->Cell(0,0,$user['Inst_Id'],0,0,'L',0);
    ...
    function frmvalidation()
    {
        ...
        else if(@$_POST['bay_form'] == '' )
        {
            ...
            ($bay_form == "0") && (@$_POST['bay_form'] != "00810-7154807-1") && (@$_POST['nationality'] == 1)
            )
            {
            ...
            else if(@$_POST['father_cnic'] == '' )
            {
                ...
                ($father_cnic == "0") && (@$_POST['father_cnic'] != "04034-2100005-1" && @$_POST['father_cnic'] != "00810-5105020-1") && (@$_POST['nationality'] == 1)
                )
                {
