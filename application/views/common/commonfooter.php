...
setTimeout(function(){count = 1;},200);
    ...
    {
        window.location.href = '<?=base_url()?>Admission_matric/cutlist/'+Batch_ID+'/3'
    }
    function RevenueForm(Batch_ID)
    {
        window.location.href = '<?=base_url()?>Admission_matric/revenue_pdf/'+Batch_ID+'/3'
    }
    ...
    $('#get_Proof_reg').click( function(){
     ...
     $('#get_proof_cutlist').click( function(){
        var option =  $('input[type=radio][name=opt]:checked').val(); 
        if(option == "1")
        {
            var std_group = $('#std_group').val();
            ...
            Print_cutlist_Proofreading_Groupwise(std_group);
        }
        else if(option =="2")
        {
            var startformno = $('#strt_formNo').val();
            ...
            Print_cutlist_Proofreading_Formnowise(startformno,endformno);
        }
        else{
            return;
        }
    ...
    function Print_Registration_Form_Proofreading_Groupwise(grp_cd)
    {
     ...
     function Print_cutlist_Proofreading_Groupwise(grp_cd)
    {
        window.location.href =  '<?=base_url()?>Admission_matric/cutlist/'+grp_cd + '/1'
    }
    ...
    function Print_cutlist_Proofreading_Formnowise(startformno,endformno)
    {
        window.location.href =  '<?=base_url()?>Admission_matric/cutlist/'+startformno + '/2' +'/'+endformno+'/';
    }
    ...
    // or
        ...
        else if ( exam_type == 16 || exam_type == 14) 
        {
            debugger;
           ...
           else if (ddlMarksImproveoptions != 4 && (exam_type == 16 || exam_type == 14))
            {
            if (sub6p1 == 0 )
        {
        $('#ErrMsg').show(); 
